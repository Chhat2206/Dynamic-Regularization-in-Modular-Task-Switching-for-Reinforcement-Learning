...
is_rq2 = not is_rq1
mode = "cyclic_task_switching"  # Options are "none", "cyclic_task_switching", "randomized_task_switching", "structured_task_switching"
goals = ["quick_recovery", "periodic_swing", "maintain_balance"]
fixed_reg_type = "l2"  # Choose from ["dropout", "l1", "l2", "batch_norm"]
...
for episode in range(num_episodes):
    ...
    is_converged_flag, avg_reward_check, std_reward_check, goal_min, goal_max = check_convergence(
        task_rewards, window_size, current_goal, reward_window, percentage=10.0
    )
...
filename = f"cartpole_results_{'rq1' if is_rq1 else f'rq2_{parameter_noise_stddev}'}_{mode if mode != 'none' else f'fixed_{fixed_reg_type}'}.xlsx"
