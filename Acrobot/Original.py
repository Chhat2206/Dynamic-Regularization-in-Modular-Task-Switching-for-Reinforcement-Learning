...
# Hyperparameters
learning_rate = 0.0005  # Slightly lower for more stable learning, OG: 0.001
gamma = 0.99
epsilon = 1.0  # Exploration rate
epsilon_decay = 0.995
min_epsilon = 0.01
batch_size = 64  # Recommended 128 or 256, prof says 32 has valuable research
...
# Reward shaping function (optional)
def shape_reward(state, next_state, reward):
    # Add a small reward if the agent gets closer to the goal
    if abs(next_state[0]) < abs(state[0]):  # Example condition for improvement
        reward += 0.1
    return reward
